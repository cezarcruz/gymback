...
class TeacherMysqlGateway implements SaveTeacherGateway, DeleteTeacherGateway, GetTeacherGateway {
  ...
  public Optional<TeacherDomain> findById(final Long id) {
    return teacherRepository.findById(id).map(teacherPersistenceMapper::toDomain);
  }
}
